...
parser.add_argument('--test_file', required=False,
                    default='data_lists/elevator_train_data_list.txt',
                    metavar="/path/to/refine weights")

parser.add_argument('--model', required=False,
                    default=ROOT_DIR + '/trained_models/elevator/elevator_2/pose_model_current.pth',
                    metavar="/path/to/weights.h5 or 'coco'")
parser.add_argument('--refine_model', required=False,
                    default=ROOT_DIR + '/trained_models/elevator/elevator_2/pose_refine_model_current.pth',
                    metavar="/path/to/weights.h5 or 'coco'")
...
parser.add_argument('--classes_train', required=False, default='classes.txt',
                    metavar="/path/to/weights.h5 or 'coco'")
parser.add_argument('--class_ids_train', required=False, default='classes_ids.txt',
                    metavar="/path/to/weights.h5 or 'coco'")
...
norm_ = transforms.Normalize(mean=[107.1515813 / 255, 108.32803021 / 255, 105.53228755 / 255],
...
for run_idx, idx in enumerate(test_idx):
    ...
    for affordance_id in affordance_ids:
        ...
        if affordance_id not in class_IDs_train:
            ...
            gt_rot = np.array(meta['rot' + meta_idx])
