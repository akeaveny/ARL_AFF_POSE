...
parser.add_argument('--dataset_config', required=False, default='/home/akeaveny/catkin_ws/src/object-rpe-ak/DenseFusion/datasets/arl/dataset_config',
                    type=str,
                    metavar="")
...
parser.add_argument('--model', required=False, default=ROOT_DIR + '/trained_models/arl/arl2/pose_model_2_0.012785316116517906.pth',
                    metavar="/path/to/weights.h5 or 'coco'")
parser.add_argument('--refine_model', required=False, default=ROOT_DIR + '/trained_models/arl/arl2/pose_refine_model_21_0.0057226678715772.pth',
                    metavar="/path/to/weights.h5 or 'coco'")
...
parser.add_argument('--output_result_dir', required=False, default=ROOT_DIR + '/experiments/eval_result/arl',
                    type=str,
                    metavar="Visualize Results")
parser.add_argument('--error_metrics_dir', required=False, default=ROOT_DIR + '/experiments/eval_result/arl/Densefusion_error_metrics_result/',
                    type=str,
                    metavar="")
parser.add_argument('--visualize', required=False, default=True,
                    type=str,
                    metavar="Visualize Results")
parser.add_argument('--save_images_path', required=False, default='/data/Akeaveny/Datasets/arl_scanned_objects/ARL/test_densefusion/',
                    type=str,
                    metavar="Visualize Results")
...
norm_ = transforms.Normalize(mean=[158.46 / 255, 151.62 / 255, 156.41 / 255],
                                 std=[40.90727774 / 255, 44.04699447 / 255, 38.46340635 / 255])  # TODO
...
for run_idx, idx in enumerate(test_idx):
    ...
    for affordance_id in affordance_ids:
        if affordance_id in class_IDs:
            ...
            print("Camera: ", meta['camera_setting' + meta_idx][0])
            ...
            try:
                ...
                gt_rot1 = np.dot(np.array(meta['rot' + np.str(meta_idx)]), np.array([[-1, 0, 0], [0, -1, 0], [0, 0, -1]]))
                gt_rot1 = np.dot(gt_rot1.T, np.array([[0, 1, 0], [1, 0, 0], [0, 0, -1]]))

                gt_trans = np.array(meta['cam_translation' + np.str(meta_idx)][0]) / 1e3  # now in [m]
                ...
                if ADD < args.ADD:
                    num_correct += 1
                    # print('{} Pass! Distance: {:.2f}'.format(classes_full[int(itemid) - 1], ADD))
                    # fw.write('{} Pass! Distance: {:.2f}'.format(classes_full[int(itemid) - 1], ADD))
                    print('{} NOT Pass! Distance: {:.2f}'.format(int(itemid) - 1, ADD))
                    fw.write('{} NOT Pass! Distance: {:.2f}'.format(int(itemid) - 1, ADD))
                else:
                    # print('{} NOT Pass! Distance: {:.2f}'.format(classes_full[int(itemid) - 1], ADD))
                    # fw.write('{} NOT Pass! Distance: {:.2f}'.format(classes_full[int(itemid) - 1], ADD))
                    print('{} NOT Pass! Distance: {:.2f}'.format(int(itemid) - 1, ADD))
                    fw.write('{} NOT Pass! Distance: {:.2f}'.format(int(itemid) - 1, ADD))
                if args.visualize:
                    plt.subplot(3, 2, 1)
