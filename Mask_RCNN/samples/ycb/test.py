...
parser.add_argument('--save_inference_images', required=False, default='/data/Akeaveny/Datasets/ycb_syn/test_maskrcnn/',
                    type=str,
                    metavar="/path/to/YCB/dataset/")
...
if args.dataset_type == 'real':
    ...
    import dataset_syn as YCB
    save_to_folder = '/images/test_images_syn/'
    MEAN_PIXEL_ = np.array([91.13, 88.92, 98.65])  ### REAL RGB
    RPN_ANCHOR_SCALES_ = (16, 32, 64, 128, 256)
    ### config ###
    MAX_GT_INSTANCES_ = 5
    DETECTION_MAX_INSTANCES_ = 5
...
def seq_get_masks(image, cur_detection, gt_mask, args):
    ...
    if cur_masks.shape[-1] > 0:
        ...
        for i in range(cur_masks.shape[-1]):
            ...
            if args.dataset_type == 'syn' or args.dataset_type == 'syn1':
                cur_class_ids[i] = cur_class_ids[i]
            # elif args.dataset_type == 'real' :
            #     cur_class_ids[i] = map_YCB_label(cur_class_ids[i])
            # print("Pred YCB Label:", cur_class_ids[i])
    ...
    #  add color to masks
    ########################
    instance_to_color = YCB.color_map()
...
def detect_and_get_masks(model, config, args):
    ...
    dataset = YCB.YCBDataset()
    dataset.load_YCB(args.dataset, args.dataset_split)
    dataset.prepare()
    ...
    for num_image, idx in enumerate(test_idx):
        ...
        # print("GT YCB Label:", np.unique(gt_label))
...
# LOOKUP FROM OBJECT ID TO YCB LABEL
###########################################################
# def map_YCB_label(current_id):
#
#     # 1
#     grasp = [
#         20,  # "hammer-grasp"
...
#         return 7
#     else:
#         print(" --- Object ID does not map to YCB Label --- ")
#         print(current_id)
#         exit(1)
...
if __name__ == '__main__':
    ...
    class InferenceConfig(YCB.YCBConfig):
        GPU_COUNT = 1
        IMAGES_PER_GPU = 1
