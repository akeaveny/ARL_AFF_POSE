...
class YCB(Config):
    ...
    # ===== dataset ======
    MEAN_PIXEL = np.array([47.88158281, 52.16680682, 67.21311318])
    ...
    STEPS_PER_EPOCH = 1540 // bs
    VALIDATION_STEPS = 380 // bs
...
def detect_and_get_masks(model, data_path, num_frames):
    file_dir = '/data/Akeaveny/Datasets/YCB_Video_Dataset/ycb_config/dataset_config/class_ids.txt'
    assign_first_pre_detect = True
    for i in range(0, num_frames):
        # assign_first_pre_detect = True

        ## ============== ZED ===================
        # folder_to_save = "/train_zed/"
        # # min_img = 000002
        # count = 1000002 + i
        # str_num = str(count)[1:]
        # rgb_addr = data_path + "/train_zed/rgb/" + str_num + "-color.png"
        # depth_addr = data_path + "/train_zed/depth/" + str_num + "-depth.png"
        ...
        # count = 1000000 + i
        # str_num = str(count)[1:]
        # rgb_addr = '/data/Akeaveny/Datasets/pringles/data_1/train/rgb/' + str_num + ".png"
        # depth_addr = '/data/Akeaveny/Datasets/pringles/data_1/train/depth/' + str_num + ".depth.png"
        # print(rgb_addr, depth_addr)

        ## ============== Combined ===================
        folder_to_save = "/train_combined/"
        min_img = 20
        count = 110 + min_img + i
        str_num = str(count)[1:]
        rgb_addr = data_path + folder_to_save + "rgb/" + str_num + "-color.png"
        depth_addr = data_path + folder_to_save + "depth/" + str_num + "-depth.png"
        print(rgb_addr, depth_addr)

        ## ============== SYNTHETIC ===================
        # min_img = 55010
        # count = 1000000 + min_img + i
        # str_num = str(count)[1:]
        # rgb_addr = data_path + "/train_combined/rgb/" + str_num + "-color.png"
        # depth_addr = data_path + "/train_combined/depth/" + str_num + "-depth.png"
        # print(rgb_addr, depth_addr)
        ...
        if not assign_first_pre_detect:
            ...
            if good_detect:
                mask_addr = data_path + folder_to_save + '/mask-color/' + str_num + '.png'
                skimage.io.imsave(mask_addr, color_masks)
                mask_addr = data_path + folder_to_save + '/mask/' + str_num + '.png'
                skimage.io.imsave(mask_addr, instance_masks)

            # ========== tutorials ================
            results = model.detect([image], verbose=1)
            r = results[0]
            file_dir = '/data/Akeaveny/Datasets/YCB_Video_Dataset/ycb_config/dataset_config/classes.txt'
            class_names = np.loadtxt(file_dir, dtype=np.str)
