...
if __name__ == '__main__':
    ...
    # image_idx = int(np.random.choice(len(dataset.image_ids), size=1))
    # image = dataset.load_image(image_idx)
    # # log("image shape", np.array(image).shape)
    # log("image shape", image)
    # mask, class_ids = dataset.load_mask(image_idx)
    # print(class_ids)
    ...
    n_rows = 2
    n_cols = 2
    axis_pairs = ([0, 0], [0, 1], [1, 0], [1, 1])
    fig, ax = plt.subplots(n_rows, n_rows, figsize=(16, 16))
    for index, (x_axis, y_axis) in enumerate(axis_pairs):
        image_idx = int(np.random.choice(len(dataset.image_ids), size=1))
        print("image_idx", image_idx)
        image, image_meta, class_ids, bbox, mask = modellib.load_image_gt(dataset, config, image_idx, use_mini_mask=False)
        log("molded_image", image)
        log("mask", mask)
        log("class_ids", class_ids)
        print("captions", dataset.class_names)
        print("captions", np.array(dataset.class_names)[class_ids].tolist())
        visualize.display_instances(image, bbox, mask, class_ids, dataset.class_names, ax=ax[x_axis, y_axis],
                                        captions=np.array(dataset.class_names)[class_ids].tolist())
    plt.savefig(os.getcwd() + "/images/real.png", bbox_inches='tight')
    plt.show()
    '''================= Histograms ====================='''
